...
- Compact the Web playback panel by merging the instrument selector into the settings row and reducing playback-only spacing and control dimensions.
- Compute jianpu vertical spacing per visual row so instrumental rows stay compact while lyric-bearing rows retain lyric clearance.
