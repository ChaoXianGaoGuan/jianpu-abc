...
- 高低八度点、单双附点。附点按其新增的时值占据独立延伸网格，数字仍位于去掉附点后的基础时值中心，底线延伸到最后一个附点。
- 以拍号分母为一拍：每个事件的横向占位与其实际时值成正比，并计入附点和三连音比例。当小节包含十六分等短音时，每拍宽度会自动扩张，在保持时值比例的同时为数字留出最小可读间距；相邻完整拍之间另外留出明确间隔。长时值展开为延音横线，短时值使用底线；同一拍内按底线层级连接 note/rest，八分与十六分会共享第一根底线，十六分额外保留第二根短线。
