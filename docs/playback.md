...
- `!D.C.!` / `!dacapo!` 会在第一次到达该标记所在小节后回到开头；`!D.S.!` 会回到 `!segno!`，没有 Segno 时退回开头。
- D.C./D.S. 回跳后会从目标位置按事件顺序扫描 `!fine!` 和 `!coda!`：谁先出现谁生效。`!fine!` 会停止；`!coda!` 只有在 D.C./D.S. 之后存在目标 Coda 段时才会跳转，否则会被忽略并继续寻找 Fine 或播到曲尾。
