...
- 保留反复导航装饰，例如 `!segno!`、`!coda!`、`!fine!`、`!D.C.!`、`!D.S.!`、`!dacapo!` 和 `!dacoda!`。这些目前只作为视觉标记导出，不展开播放顺序。
...
- 非默认时值输出相对 `L:` 的倍率，例如在 `L:1/4` 下，八分音符输出 `1/2` 后缀；绝对时值字母会先解析为固定时值再导出。
...
仅支持大调。多声部会导出为独立 `V:` 段落，但尚不处理 ABC `%%score` 分组或同谱表合并。ABC 导出保留反复和 ending 符号；播放层会展开简单反复，但导出器本身不改写乐谱结构。D.C./D.S./Fine/Coda 当前只保留为 ABC 装饰文本，不做语义展开。暂不处理跨小节延音，也不导出装饰音、连线或和弦。新增规则必须同时增加 golden 或单元测试。
