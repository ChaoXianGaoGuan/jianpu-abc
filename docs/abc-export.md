...
仅支持大调。多声部会导出为独立 `V:` 段落，但尚不处理 ABC `%%score` 分组或同谱表合并。ABC 导出保留反复、ending 和反复导航装饰；播放层会展开简单反复、一二房子以及常见 D.C./D.S. 到 Fine/Coda，但导出器本身不改写乐谱结构。暂不处理跨小节延音，也不导出装饰音、连线或和弦。新增规则必须同时增加 golden 或单元测试。
